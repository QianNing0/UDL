...
def b_GPUVar_Nearest(variable, scale):
    tensor = variable.cpu().data
    B, C, H, W = tensor.size()
    H_new = int(H / scale)
    W_new = int(W / scale)
...
def b_CPUVar_Nearest(variable, scale):
    tensor = variable.data
    B, C, H, W = tensor.size()
    H_new = int(H / scale)
    W_new = int(W / scale)
...
class Preprocessing(object):
    def __init__(self, scale,  para_input=10, kernel=21, noise=True, cuda=False, sig_min=0.2, sig_max=4.0,
                 rate_iso=1.0, scaling=3, rate_cln=0.2, noise_high=0.08, AT=False,downsample='bicubic'):
    ...
    def __call__(self, hr_tensor, kernel=False):
        ...
        #     else:
        #         lr_blured_AT = lr_blured_t
        # elif self.downsampler == 'nearest':
        #     if self.cuda:
        #         lr_blured_t = b_GPUVar_Nearest(hr_blured_var, self.scale)
        #     else:
        #         lr_blured_t = b_CPUVar_Nearest(hr_blured_var, self.scale)
        # # A matrix transpose
        #     if self.AT:
        #         if self.cuda:
        #             lr_blured_DT = b_GPUVar_Nearest(lr_blured_t, 1/self.scale)
        #             lr_blured_AT = self.blur(Variable(lr_blured_DT).cuda(),b_kernels)
        #         else:
        #             lr_blured_DT = b_CPUVar_Nearest(lr_blured_t, 1/self.scale)
        #             lr_blured_AT = self.blur(lr_blured_DT, b_kernels)
        #     else:
        #         lr_blured_AT = lr_blured_t
        # # Noisy
